...
router.prefix('/api/check_sheet_item')

router.get('/list', async (ctx, next) => {
  const data = await get('check_sheet_item', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    ...
    idKey: 'c_item_id',
    createTimeKey: 's_item_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('check_sheet_item', 'c_item_id', id);
