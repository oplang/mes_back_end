...
router.prefix('/api/wip_product_history')

router.get('/list', async (ctx, next) => {
  const data = await get('wip_product_history', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'wip_product_history',
    tableForm: ctx.request.body,
    idKey: 'w_prod_timekey',
    createTimeKey: 'w_prod_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('wip_product_history', 'w_prod_timekey', id);
