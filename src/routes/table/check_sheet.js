...
router.prefix('/api/check_sheet')

router.get('/list', async (ctx, next) => {
  const data = await get('check_sheet', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    ...
    idKey: 'c_sheet_id',
    createTimeKey: 'c_sheet_create_time'
  });
...
router.put('/update', async (ctx) => {
  ...
  const result = await update({
    ...
    tableForm: ctx.request.body,
    idKey: 'c_sheet_id',
    editTimeKey: 'c_sheet_event_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('check_sheet', 'c_sheet_id', id);
