...
router.prefix('/api/sys_user')

router.get('/list', async (ctx, next) => {
  const data = await get('sys_user', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    ...
    idKey: 'user_id',
    createTimeKey: 'user_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('sys_user', 'user_id', id);
