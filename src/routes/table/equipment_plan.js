...
router.prefix('/api/equipment_plan')

router.get('/list', async (ctx, next) => {
  const data = await get('equipment_plan', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'equipment_plan',
    tableForm: ctx.request.body,
    idKey: 'e_plan_id',
    createTimeKey: 'e_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('equipment_plan', 'e_plan_id', id);
