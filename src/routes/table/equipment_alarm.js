...
router.prefix('/api/equipment_alarm')

router.get('/list', async (ctx, next) => {
  const data = await get('equipment_alarm', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    ...
    idKey: 'alarm_id',
    createTimeKey: 'alarm_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('equipment_alarm', 'alarm_id', id);
