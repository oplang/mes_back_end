...
router.prefix('/api/equipment_check_item')

router.get('/list', async (ctx, next) => {
  const data = await get('equipment_check_item', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'equipment_check_item',
    tableForm: ctx.request.body,
    idKey: 'check_id',
    createTimeKey: 'check_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('equipment_check_item', 'check_id', id);
