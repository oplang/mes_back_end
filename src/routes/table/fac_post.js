...
router.prefix('/api/fac_post')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_post', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    ...
    idKey: 'post_id',
    createTimeKey: 'post_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_post', 'post_id', id);
