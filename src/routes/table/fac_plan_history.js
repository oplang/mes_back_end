...
router.prefix('/api/fac_plan_history')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_plan_history', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_plan_history',
    tableForm: ctx.request.body,
    idKey: 'plan_timekey',
    createTimeKey: 'plan_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_plan_history', 'plan_timekey', id);
