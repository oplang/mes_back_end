...
router.prefix('/api/fac_bulletin')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_bulletin', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_bulletin',
    tableForm: ctx.request.body,
    idKey: 'bul_id',
    createTimeKey: 'bul_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_bulletin', 'bul_id', id);
