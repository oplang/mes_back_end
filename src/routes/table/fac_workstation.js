...
router.prefix('/api/fac_workstation')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_workstation', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    ...
    idKey: 'station_id',
    createTimeKey: 'station_create_time'
  });
...
router.put('/update', async (ctx) => {
  ...
  const result = await update({
    ...
    tableForm: ctx.request.body,
    idKey: 'station_id',
    editTimeKey: 'station_event_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_workstation', 'station_id', id);
