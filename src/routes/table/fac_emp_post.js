...
router.prefix('/api/fac_emp_post')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_emp_post', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_emp_post',
    tableForm: ctx.request.body,
    idKey: 'emp_id',
    createTimeKey: 'ep_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_emp_post', 'emp_id', id);
