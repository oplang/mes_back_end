...
router.prefix('/api/product_info')

router.get('/list', async (ctx, next) => {
  const data = await get('product_info', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    ...
    idKey: 'prod_id',
    createTimeKey: 'prod_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('product_info', 'prod_id', id);
