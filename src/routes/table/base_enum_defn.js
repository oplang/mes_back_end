...
router.prefix('/api/base_enum_defn')

router.get('/list', async (ctx, next) => {
  const data = await get('base_enum_defn', ctx.query);
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('base_enum_defn', 'enum_name', id);
