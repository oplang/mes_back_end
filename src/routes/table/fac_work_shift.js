...
router.prefix('/api/fac_work_shift')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_work_shift', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_work_shift',
    tableForm: ctx.request.body,
    idKey: 'shift_id',
    createTimeKey: 'shift_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_work_shift', 'shift_id', id);
