...
router.prefix('/api/equipment_exception')

router.get('/list', async (ctx, next) => {
  const data = await get('equipment_exception', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'equipment_exception',
    tableForm: ctx.request.body,
    idKey: 'excep_id',
    createTimeKey: 'excep_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('equipment_exception', 'excep_id', id);
