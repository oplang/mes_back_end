...
router.prefix('/api/fac_plan_item')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_plan_item', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_plan_item',
    tableForm: ctx.request.body,
    idKey: 'p_item_id',
    createTimeKey: 'p_item_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_plan_item', 'p_item_id', id);
