...
router.prefix('/api/fac_department')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_department', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_department',
    tableForm: ctx.request.body,
    idKey: 'dept_id',
    createTimeKey: 'dept_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_department', 'dept_id', id);
