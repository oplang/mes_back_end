...
router.prefix('/api/material_delivery_list')

router.get('/list', async (ctx, next) => {
  const data = await get('material_delivery_list', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'material_delivery_list',
    tableForm: ctx.request.body,
    idKey: 'del_id',
    createTimeKey: 'del_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('material_delivery_list', 'del_id', id);
