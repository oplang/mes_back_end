...
router.prefix('/api/fac_calendar')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_calendar', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_calendar',
    tableForm: ctx.request.body,
    idKey: 'cal_id',
    createTimeKey: 'cal_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_calendar', 'cal_id', id);
