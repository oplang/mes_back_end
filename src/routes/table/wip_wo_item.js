...
router.prefix('/api/wip_wo_item')

router.get('/list', async (ctx, next) => {
  const data = await get('wip_wo_item', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'wip_wo_item',
    tableForm: ctx.request.body,
    idKey: 'w_item_id',
    createTimeKey: 'w_item_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('wip_wo_item', 'w_item_id', id);
