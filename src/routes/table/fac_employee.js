...
router.prefix('/api/fac_employee')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_employee', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_employee',
    tableForm: ctx.request.body,
    idKey: 'emp_id',
    createTimeKey: 'emp_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_employee', 'emp_id', id);
