...
router.prefix('/api/base_reason_defn')

router.get('/list', async (ctx, next) => {
  const data = await get('base_reason_defn', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'base_reason_defn',
    tableForm: ctx.request.body,
    idKey: 'reason_id',
    createTimeKey: 'reason_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('base_reason_defn', 'reason_id', id);
