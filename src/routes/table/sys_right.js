...
router.prefix('/api/sys_right')

router.get('/list', async (ctx, next) => {
  const data = await get('sys_right', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    ...
    idKey: 'right_id',
    createTimeKey: 'role_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('sys_right', 'right_id', id);
