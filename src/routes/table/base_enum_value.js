...
router.prefix('/api/base_enum_value')

router.get('/list', async (ctx, next) => {
  const data = await get('base_enum_value', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'base_enum_value',
    tableForm: ctx.request.body,
    idKey: 'value_id',
    createTimeKey: 'value_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('base_enum_value', 'value_id', id);
