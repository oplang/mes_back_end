...
router.prefix('/api/check_model')

router.get('/list', async (ctx, next) => {
  const data = await get('check_model', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'check_model',
    tableForm: ctx.request.body,
    idKey: 'model_id',
    createTimeKey: 'model_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('check_model', 'model_id', id);
