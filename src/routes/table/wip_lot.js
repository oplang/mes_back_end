...
router.prefix('/api/wip_lot')

router.get('/list', async (ctx, next) => {
  const data = await get('wip_lot', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    ...
    idKey: 'w_lot_id',
    createTimeKey: 'w_lot_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('wip_lot', 'w_lot_id', id);
