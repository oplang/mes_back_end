...
router.prefix('/api/material_abnormal_list')

router.get('/list', async (ctx, next) => {
  const data = await get('material_abnormal_list', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'material_abnormal_list',
    tableForm: ctx.request.body,
    idKey: 'abn_id',
    createTimeKey: 'abn_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('material_abnormal_list', 'abn_id', id);
