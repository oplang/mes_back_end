...
router.prefix('/api/check_model_item')

router.get('/list', async (ctx, next) => {
  const data = await get('check_model_item', ctx.query);
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('check_model_item', 'c_item_id', id);
