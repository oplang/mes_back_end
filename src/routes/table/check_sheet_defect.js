...
router.prefix('/api/check_sheet_defect')

router.get('/list', async (ctx, next) => {
  const data = await get('check_sheet_defect', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'check_sheet_defect',
    tableForm: ctx.request.body,
    idKey: 'defect_id',
    createTimeKey: 'defect_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('check_sheet_defect', 'defect_id', id);
