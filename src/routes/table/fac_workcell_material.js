...
router.prefix('/api/fac_workcell_material')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_workcell_material', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_workcell_material',
    tableForm: ctx.request.body,
    idKey: 'station_id',
    createTimeKey: 'cell_mat_create_time'
  });
...
router.put('/update', async (ctx) => {
  ...
  const result = await update({
    ...
    tableForm: ctx.request.body,
    idKey: 'station_id',
    editTimeKey: 'cell_mat_event_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_workcell_material', 'station_id', id);
