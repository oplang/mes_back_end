...
router.prefix('/api/fac_workteam')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_workteam', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_workteam',
    tableForm: ctx.request.body,
    idKey: 'team_id',
    createTimeKey: 'team_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_workteam', 'team_id', id);
