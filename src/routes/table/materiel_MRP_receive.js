...
router.prefix('/api/materiel_MRP_receive')

router.get('/list', async (ctx, next) => {
  const data = await get('materiel_MRP_receive', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'materiel_MRP_receive',
    tableForm: ctx.request.body,
    idKey: 'receive_id',
    createTimeKey: 'receive_create_time'
  });
...
router.put('/update', async (ctx) => {
  ...
  const result = await update({
    ...
    tableForm: ctx.request.body,
    idKey: 'receive_id',
    editTimeKey: 'receive_event_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('materiel_MRP_receive', 'receive_id', id);
