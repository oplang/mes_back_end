...
router.prefix('/api/materiel_lot')

router.get('/list', async (ctx, next) => {
  const data = await get('materiel_lot', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'materiel_lot',
    tableForm: ctx.request.body,
    idKey: 'lot_id',
    createTimeKey: 'lot_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('materiel_lot', 'lot_id', id);
