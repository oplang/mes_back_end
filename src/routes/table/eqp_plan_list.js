...
router.prefix('/api/eqp_plan_list')

router.get('/list', async (ctx, next) => {
  const data = await get('eqp_plan_list', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'eqp_plan_list',
    tableForm: ctx.request.body,
    idKey: 'e_plan_id',
    createTimeKey: 'list_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('eqp_plan_list', 'e_plan_id', id);
