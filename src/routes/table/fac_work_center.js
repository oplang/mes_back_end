...
router.prefix('/api/fac_work_center')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_work_center', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_work_center',
    tableForm: ctx.request.body,
    idKey: 'center_id',
    createTimeKey: 'center_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_work_center', 'center_id', id);
