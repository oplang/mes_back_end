...
router.prefix('/api/equipment_spec_checkitem')

router.get('/list', async (ctx, next) => {
  const data = await get('equipment_spec_checkitem', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'equipment_spec_checkitem',
    tableForm: ctx.request.body,
    idKey: 's_check_id',
    createTimeKey: 's_check_create_time'
  });
...
router.put('/update', async (ctx) => {
  ...
  const result = await update({
    ...
    tableForm: ctx.request.body,
    idKey: 's_check_id',
    editTimeKey: 's_check_event_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('equipment_spec_checkitem', 's_check_id', id);
