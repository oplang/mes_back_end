...
router.prefix('/api/fac_calendar_item')

router.get('/list', async (ctx, next) => {
  const data = await get('fac_calendar_item', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'fac_calendar_item',
    tableForm: ctx.request.body,
    idKey: 'citem_id',
    createTimeKey: 'citem_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('fac_calendar_item', 'citem_id', id);
