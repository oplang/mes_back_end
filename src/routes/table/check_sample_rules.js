...
router.prefix('/api/check_sample_rules')

router.get('/list', async (ctx, next) => {
  const data = await get('check_sample_rules', ctx.query);
...
router.post('/add', async (ctx) => {
  ...
  const result = await add({
    tableName: 'check_sample_rules',
    tableForm: ctx.request.body,
    idKey: 'rule_id',
    createTimeKey: 'rule_create_time'
  });
...
router.delete('/del', async (ctx) => {
  ...
  const result = await del('check_sample_rules', 'rule_id', id);
